...
- Note: we denote the expected value with the $E[\cdot]$ operator. Hence, $E[u_{i}(\cdot)]$(or $E[\pi_i(\cdot)]$) denote expected payoffs.

- What is $E[u_S(\sigma_S, H)]$? $E[u_S(\sigma_S, T)]$?
...
- Given this, Bean is thinking about playing a mixed strategy as well. When does it make sense to do so?
   - Let's compare $E[u_B(\sigma_S, H)]$ vs. $E[u_B(\sigma_S, T)]$
