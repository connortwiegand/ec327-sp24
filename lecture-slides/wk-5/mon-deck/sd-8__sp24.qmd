...
- Ann
   - $|S_A| = 2\cdot 2\cdot3\cdot3\cdot2 = 54$

- Beth
   - $|S_B| = 2\cdot 2\cdot2\cdot2\cdot2\cdot2 = 12$
...
:::{.panel-tabset}
#### Extensive Form
![](imgs/entrant-game.png){width=60%}

#### Normal Form
![](imgs/sfg-ent.png){width=50%}
...
- In fact, given an extensive form game:
   ...
   - Need SPNE?
      ...
      - _Method:_ [Backward Induction]{.hi}^[IMHO this is a dumb name. Then again, Dixit, Skeath, and McAdams use the term _Rollback Method_ and _Rollback Equilibrium_, so there are worse things]

### Backward Induction

- For now, we will assume singleton info. sets (no dashed lines)

- Like too much of game theory, this method is hard to write down, but easy to display and easy to perform once you get the hang of it

- Let's do an example: Entrant Game
...
### SPNE: Kidnapper Game (cont.)


### SPNE: Kidnapper Game w/ Imperfect Info.


### SPNE: Kidnapper Game w/ Imperfect Info. (cont.)
...
- [Remark:]{.alert} like IDSDS, it may be easiest to _replace_ each subgame with the payoff achieved from following your marked path (see next slide)
