...
- Suppose the [interest rate]{.alert} is 25%: you would be indifferent between \$1 now and \$1.25 in the next [**period**]{.hi}  
   ...
   - What does that mean?
      ...
      - In this world suppose we asked "what is \$1.25 received tomorrow worth today?"



### Discounting, cont.

- [$r$]{.alert} = 25%: indifferent between \$1 now and \$1.25 in the next [period]{.hi}  
   - If $r$ is the interest rate, then $\delta$ is given by
      $$ \delta = \frac{1}{1+0.25}=0.8$$

- What does that mean?

   - In this world, suppose we asked "what is \$1.25 received tomorrow worth today?"

   - $\delta$ is the discount [factor]{.uli} is because it's what we multiply '\$1.25 tomorrow' by to get it's worth today
      - e.g. $$(\$1.25)\delta = \$1.25(0.8)=\$1$$ 
\
...
- Then the _series_ from $1$ to $k$ is a sum of the terms of this sequence, denoted by the following
   ...
   $$ \sum_{k=1}^{n}x_{k}= x_{1} + x_{2} + x_3 + \cdots + x_{n}$$
